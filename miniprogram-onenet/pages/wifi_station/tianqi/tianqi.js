...
const devicesId = "1172729201" // 填写在OneNet上获得的devicesId 形式就是一串数字 例子:9939133
const api_key = "LgU42xT1gzq5nwQFFcUU5AJfGUM=" // 填写在OneNet上的 api-key 例子: VeFI0HZ44Qn5dZO14AuLbWSlSlI=
...
Page({
  ...
  update: function (datapoints) {
    ...
    if(wheatherData.humidity[wheatherData.humidity.length-1]>93){
    this.setData({
      hum: wheatherData.humidity[wheatherData.humidity.length - 1],
      tem: wheatherData.tempe[wheatherData.tempe.length - 1],
      status1:"停止浇水"
    })}else{
      this.setData({
        hum: wheatherData.humidity[wheatherData.humidity.length - 1],
